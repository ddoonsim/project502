...
public class ConfigSaveTest {
    ...
    void saveTest() {
        ...
        // JSON데이터를 단일 자바 클래스로 변환
        BasicConfig config2 = infoService.get("basic", BasicConfig.class).get() ;
        System.out.println("BasicConfig의 형태 : " + config2);

        // JSON데이터를 복합적인 형태의 자료형으로 변환
        Optional<Map<String, String>> config3 = infoService.get("basic", new TypeReference<>() {});
        System.out.println("Map 형태 : " + config3.get());
    }
}
